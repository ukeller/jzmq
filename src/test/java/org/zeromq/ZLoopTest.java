...
public class ZLoopTest {
	...
	ZLoop.IZLoopHandler socketEvent = new ZLoop.IZLoopHandler() {
	    ...
	    @Override
	    public int handle(ZLoop loop, PollItem item, Object arg) {
		received = ((Socket) arg).recvStr(0, Charset.defaultCharset());
    ...
    @Test
    public void testZLoopTimerOnly() {
	// test to reproduce NPE in ZLoop#start when dirty is false on first loop and
	// have timers only
	ZLoop loop = new ZLoop();
	final AtomicBoolean timerFired = new AtomicBoolean(false);

	loop.addTimer(1, 1, new IZLoopHandler() {
	    @Override
	    public int handle(ZLoop loop, PollItem item, Object arg) {
		timerFired.set(true);
		// exit
	...
	loop.start();
	Assert.assertTrue(timerFired.get());
